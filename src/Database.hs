...
merge :: Opts -> AudiobookPathQueryResult -> AudiobookMetadataQueryResult -> Audiobook
merge opts l r = Audiobook (apqId l) 
                    (amqTitle r) 
                    (read $ apqFormat l) 
...
getBooks opts conn = do
    -- Filter relevant formats and join with data table to get path to book
    apqBooks <- query_ conn "SELECT b.id, MAX(d.format), b.path, d.name \
                \ FROM books AS b \
    ...
    amqBooks <- query conn "SELECT b.id, b.title, a.name \
                \ FROM books AS b \
                \ LEFT JOIN books_authors_link AS l ON b.id=l.book \
                \ LEFT JOIN authors AS a ON a.id=l.author\
                \ WHERE b.id IN (?) \
                \ ORDER BY b.id" (map apqId apqBooks) :: IO [AudiobookMetadataQueryResult]
